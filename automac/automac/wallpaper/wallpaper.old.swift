...
func loadConfig() -> WallpaperConfig? {
    ...
    print("Saving config: \(configURL)")
    do {
        let data = try Data(contentsOf: configURL)
        let decoder = JSONDecoder()
        return try decoder.decode(WallpaperConfig.self, from: data)
    } catch {
        print("load Failed to load or parse the config file: \(error.localizedDescription)")
        return nil
    }
}
...
func saveConfig(_ config: WallpaperConfig) {
    ...
    do {
        ...
        print("save Failed to save the config file: \(error.localizedDescription)")
    }
}
